...
std::vector<EnumeratedDevice> enumerate_matching_devices(
    ...
    if (device_count < 0)
        throw device::ConnectionError(fmt::format(
...
private:
    ...
    bool select_device(uint16_t vendor_id, int32_t product_id, const char* serial_number) {
        ...
        if (devices_opened.size() != 1) {
            ...
            logger_.error(
                ...
                devices_opened.size() ? fmt::format("{} devices", devices_opened.size()).c_str()
                                      : "No device",
                vendor_id,
                product_id >= 0 ? fmt::format(", product id (0x{:04x})", product_id).c_str() : "",
                serial_number ? fmt::format(", serial number ({})", serial_number).c_str() : "");
    ...
    int print_matched_unmatched_devices(
        ...
        for (ssize_t i = 0; i < device_count; i++) {
            ...
            const auto device_str = fmt::format(
        ...
        if (ret != 0) [[unlikely]] {
            ...
            if (error_callback_) {
                if (ret == LIBUSB_ERROR_NO_DEVICE)
                    ...
                    error_callback_(
                        fmt::format("Failed to re-submit receive transfer: {} ({})", ret,
                                    libusb_errname(ret)));
            }
        }
    }
...
std::vector<std::string>
    ...
    if (ret != 0)
        throw device::ConnectionError(
            fmt::format("Failed to init libusb: {} ({})", ret, libusb_errname(ret)));
