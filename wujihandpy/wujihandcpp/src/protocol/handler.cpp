    ...
    std::vector<uint8_t> raw_sdo_read(
        ...
        std::vector<uint8_t> result;
        {
            ...
            if (state == RawSdoUnit::State::FAILED) {
                ...
                throw device::TimeoutError(fmt::format(
    ...
    void raw_sdo_write(
        ...
        if (size != 1 && size != 2 && size != 4 && size != 8)
            throw std::invalid_argument(
                fmt::format("Raw SDO write data size must be 1, 2, 4, or 8 bytes, got {}", size));
...
private:
    ...
    void receive_transfer_completed_callback(const std::byte* buffer, size_t size) {
        ...
        try {
            ...
            if (header.type == 0x21)
                ...
                throw std::runtime_error{fmt::format("Invalid header type: 0x{:02X}", header.type)};
    ...
    void read_sdo_frame(const std::byte*& pointer, const std::byte* sentinel) {
        while (pointer < sentinel) {
            ...
            if (control == 0x35)
                ...
                throw std::runtime_error(
                    fmt::format("Invalid SDO command specifier: 0x{:02X}", control));
        }
    }
    ...
    void read_pdo_frame(const std::byte*& pointer, const std::byte* sentinel) {
        ...
        if (header.read_id == 0x01) {
            ...
            throw std::runtime_error(
                fmt::format("PDO frame invalid: read_id == 0x{:02X}", header.read_id));
    }
