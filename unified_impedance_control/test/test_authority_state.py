...
def test_y_toggles_only_on_rising_edges() -> None:
    ...
    assert state.update_buttons({"Y": True}, takeover_button="Y", start_button="A", stop_button="B")
    assert state.snapshot()["authority"] == "teleop"
    assert not state.update_buttons({"Y": True}, takeover_button="Y", start_button="A", stop_button="B")
    assert not state.update_buttons({"Y": False}, takeover_button="Y", start_button="A", stop_button="B")
    assert state.update_buttons({"Y": [1.0]}, takeover_button="Y", start_button="A", stop_button="B")
    assert state.snapshot()["authority"] == "inference"
